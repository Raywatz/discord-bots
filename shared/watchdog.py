...
def _bot_dir(bot_file):
    """Each bot lives in its own subdirectory, e.g. counting_bot.py -> counting-bot/."""
    return os.path.join(REPO_ROOT, bot_file[:-3].replace("_", "-"))


def watch_bot(bot_file):
    delay = RESTART_DELAY
    bot_dir = _bot_dir(bot_file)

...
async def _fetch_playlist_url(url: str) -> tuple[str, list[dict]]:
    ...
    loop = asyncio.get_event_loop()
    # ignoreerrors so a single unavailable/private/age-restricted video in the
    # playlist doesn't abort extraction of the entire playlist.
    opts = {**_YTDL_COMMON, "extract_flat": False, "noplaylist": False, "ignoreerrors": True}
    try:
        raw = await loop.run_in_executor(
...
async def play_next(guild: discord.Guild) -> None:
    ...
    # Serialize against concurrent invocations (e.g. two users triggering
    # playback at once while idle) so we never call vc.play() twice for the
    # same guild, which would raise and desync state.current from the queue.
    async with state.play_lock:
        ...
        try:
            ...
            if state.consecutive_failures >= 5:
                ...
                return
            # Schedule as a new task rather than awaiting recursively: an
            # awaited recursive call here would deadlock on state.play_lock
            # (which this call already holds), and on a long run of bad
            # tracks a direct recursive await could also grow the call stack
            # unbounded.
...
async def _connect_or_move(interaction: discord.Interaction, state: GuildMusicState) -> None:
    ...
    Guarded by a per-guild lock since the check-then-connect isn't atomic
    across the await — two near-simultaneous play commands (e.g. a user
    double-tapping /play) could otherwise both see vc is None and both call
    connect(), and the second raises "Already connected to a voice channel"."""
...
async def _ensure_same_channel(interaction: discord.Interaction, state: GuildMusicState) -> bool:
...
async def cmd_stop(interaction: discord.Interaction) -> None:
    ...
    state.loop = False
    state.current = None
    state.stopping = True  # tell play_next's after_play not to announce "queue finished"
    state.vc.stop()
...
async def on_voice_state_update(
    ...
    if not non_bots:
        channel = state.vc.channel
        await asyncio.sleep(30)
        # Re-check state.vc itself, not just membership — /leave or /join
        # (a channel move) during the 30s sleep can make it None or point
        # somewhere else, and touching the stale reference would crash here.
        if state.vc is None or not state.vc.is_connected() or state.vc.channel != channel:
            return
        non_bots = [m for m in state.vc.channel.members if not m.bot]
        if not non_bots:
...
async def on_app_command_error(
    ...
    # NOTE: app command errors must be registered via bot.tree.error, not
    # @bot.event — "on_app_command_error" is not a dispatched gateway event,
    # so a @bot.event handler with this name would silently never run.
    msg        = str(error)

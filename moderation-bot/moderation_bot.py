...
async def kick(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason given"):
    ...
    try:
        ...
        # Only notify the user after the kick actually succeeds — DMing "you
        # have been kicked" and then failing to kick them would be misleading.
        try:
...
async def ban(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason given", delete_days: int = 0):
    ...
    try:
        ...
        # Only notify the user after the ban actually succeeds — DMing "you
        # have been banned" and then failing to ban them would be misleading.
        try:
...
async def viewer(interaction: discord.Interaction, member: discord.Member):
    ...
    # Looping set_permissions() over every text channel below can easily take
    # longer than Discord's 3-second interaction ack window on large servers.
    await interaction.response.defer(ephemeral=True)
        ...
        if entry["action"] == "unban":
            ...
            if not entry.get("channel_id") and not entry.get("channel_name"):  # skip channel lookup for unban entries
                ...
                if not ch and entry.get("channel_name"):
                    # Fall back to name lookup for entries scheduled before
                    # channel_id was tracked (e.g. across an upgrade/restart).
...
async def on_ready():
    ...
    # Critical init (heartbeat, perm-schedule recovery) must not be gated behind
    # tree.sync() — a transient sync failure (rate limit, network hiccup) would
    # otherwise raise and prevent them from ever running.
...
async def on_message(message):
    ...
    # Restricted words — match as whole words so a restricted word isn't
    # falsely triggered by appearing as a substring of an unrelated word
    # (e.g. restricting "ass" shouldn't delete every message containing "class").
